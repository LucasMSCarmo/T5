...
export default function RelatorioTopValor(props: Props) {
                            ...
                            {dados.map((item: any, index: number) => (
                                <div key={index} className="list-group-item d-flex justify-content-between align-items-center py-2">
                                    <div>
                                        ...
                                        <div className="text-muted small">{item[metricaQtd]} itens</div>
                                    </div>
                                    <div className="d-flex align-items-center">
                                        <span className={`badge bg-${corBadge} rounded-pill me-2`}>R$ {item[metricaValor].toFixed(2)}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
    ...
    return (
        <div className="card border-0 shadow-sm">
            <div className="card-header py-3" style={{ backgroundColor: tema, color: 'white' }}>
                <h5 className="mb-0 fw-bold"><i className="bi bi-graph-up-arrow me-2"></i>Top 5 Clientes por Valor Gasto (R$)</h5>
            </div>
